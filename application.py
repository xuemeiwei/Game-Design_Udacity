...
class HangmanApi(remote.Service):
    ...
    def make_move(self, request):
        ...
        if failed == 0:
          game.end_game(True)
          return game.to_game_form('You win!')
    ...
    def get_game_history(self, request):
        """Returns a Game's history, namely all the moves"""
        game = get_by_urlsafe(request.urlsafe_game_key, Game)
        if not game:
    ...
    def cancel_game(self, request):
      ...
      if game and not game.game_over:
        ...
        raise endpoints.BadRequestException('Sorry cannot delete completed games')
      else:
    ...
    def get_highest_scores(self, request):
        """Return scores graded out from highest to lowest"""
